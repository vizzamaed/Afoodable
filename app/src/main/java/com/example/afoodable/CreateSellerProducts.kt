...
class CreateSellerProducts : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val activityResultLauncher = registerForActivityResult<Intent, ActivityResult>(
            ...
            if(result.resultCode== RESULT_OK){
                ...
                Toast.makeText(this@CreateSellerProducts, "No Image Selected", Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun uploadData(){
        ...
        FirebaseDatabase.getInstance().getReference("Inventory").child(itemName)
            .setValue(dataClass).addOnCompleteListener{task ->
                if(task.isSuccessful){
                    Toast.makeText(this@CreateSellerProducts, "Saved",Toast.LENGTH_SHORT).show()
                    finish()
                }
            }.addOnFailureListener { e ->
                Toast.makeText(
                    this@CreateSellerProducts, e.message.toString(), Toast.LENGTH_SHORT).show()
            }
    }
}
